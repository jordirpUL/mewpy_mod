    ...
    for interaction in interactions:
        ...
        for coefficient, regulatory_event in interaction.regulatory_events.items():
            ...
            if not regulatory_event.is_none:
                ...
                dfs.append(df)
        try:
            _ = real_initial_state[interaction.target.id]
        except KeyError:
            real_initial_state[interaction.target.id] = df.loc[interaction.target.id].result
    df = pd.concat(dfs)
    result_col = df.pop('result')
    df = pd.concat([result_col, df], axis=1)
    return df

def get_real_initial_state(model: Union['Model', 'MetabolicModel', 'RegulatoryModel'],
                           interactions: Sequence[str] = None,
                           initial_state: Dict[str, float] = None,
                           strategy: str = 'max',
                           operators: Union[Dict[Type[Symbolic], Callable], Dict[Type[Symbolic], Any]] = None,
    ...
    for interaction in interactions:
        ...
        for coefficient, regulatory_event in interaction.regulatory_events.items():
            #print("Interaction:",interaction,"coefficient:",coefficient)
            if not regulatory_event.is_none:
                df = regulatory_event.truth_table(values=real_initial_state,
                                                  strategy=strategy,
                                                  coefficient=coefficient,
